...
int Pipe::accept()
{
  ...
  if (r < 0) {
    ldout(msgr->cct,0) << "accept failed to getpeername " << errno << " " << cpp_strerror(errno) << dendl;
    goto fail_unlocked;
  }
...
void Pipe::fault(bool onread)
{
  ...
  ldout(msgr->cct,2) << "fault " << errno << ": " << cpp_strerror(errno) << dendl;
...
int Pipe::tcp_write(const char *buf, int len)
{
  ...
  while (len > 0) {
    ...
    if (did < 0) {
      //lgeneric_dout(cct, 1) << "tcp_write error did = " << did << "  errno " << errno << " " << cpp_strerror(errno) << dendl;
      //lgeneric_derr(cct, 1) << "tcp_write error did = " << did << "  errno " << errno << " " << cpp_strerror(errno) << dendl;
      return did;
    }
    len -= did;
    buf += did;
